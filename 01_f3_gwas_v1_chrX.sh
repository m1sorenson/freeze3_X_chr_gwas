...
do
  ...
  then
    ...
    --export=ALL,study="$study_1",cov="$cov",study_2="$study_2",ancgroup="$ancgroup",sex="$sex" run_trauma_gwas_v2_freeze3_chrX.slurm -D $WORKING_DIR
  fi
done
...
#Already created x chromosome files in 00_copy_sumstats_chrX.sh, just need to copy into bychr folder

for x_file in $(ls sumstats/*chrX*); do
  cp ${x_file} sumstats/bychr
done
...
if [ $RUNTYPE == "broad" ]; then
  ...
  sed s/{EXT1}//g metal_templates/f3_gwas_base_template.mi | \
    sed s/{EXT2}/broad_chrX/g | sed s/.chr{CHR_NUM}//g \
    > metal_templates/f3_gwas_broad_chrX.mi
  # Fill in weights
  while IFS= read -r line; do
    ...
    if [ $study_id != "study_id" ]; then
      ...
      sed s/{${study_id}_WEIGHT}/${weight}/g metal_templates/f3_gwas_broad_chrX.mi \
        | sed s/{${study_id}_FREQ}/${freq_col}/g \
        > metal_templates/f3_gwas_broad_chrX_tmp.mi
      mv metal_templates/f3_gwas_broad_chrX_tmp.mi metal_templates/f3_gwas_broad_chrX.mi
    fi
  done < config/study_weights_chrX.tsv
  ...
  sed s/{EXT1}/_males/g metal_templates/f3_gwas_base_template.mi | \
    sed s/{EXT2}/males_chrX/g | sed s/.chr{CHR_NUM}//g \
    > metal_templates/f3_gwas_males_chrX.mi
  # Fill in weights
  while IFS= read -r line; do
    study_id=$(echo "$line" | awk 'BEGIN{FS="\t"}{print $2}')
    if [ $study_id != "study_id" ]; then
      ...
      echo {${study_id}_WEIGHT} $weight $freq_col
      sed s/{${study_id}_WEIGHT}/${weight}/g metal_templates/f3_gwas_males_chrX.mi \
        | sed s/{${study_id}_FREQ}/${freq_col}/g \
        > metal_templates/f3_gwas_males_chrX_tmp.mi
      mv metal_templates/f3_gwas_males_chrX_tmp.mi metal_templates/f3_gwas_males_chrX.mi
    fi
  done < config/study_weights_chrX.tsv
elif [ $RUNTYPE == "females" ]; then
  ...
  sed s/{EXT1}/_females/g metal_templates/f3_gwas_base_template.mi | \
    sed s/{EXT2}/females_chrX/g | sed s/.chr{CHR_NUM}//g \
    > metal_templates/f3_gwas_females_chrX.mi
  # Fill in weights
  while IFS= read -r line; do
    study_id=$(echo "$line" | awk 'BEGIN{FS="\t"}{print $2}')
    if [ $study_id != "study_id" ]; then
      ...
      echo {${study_id}_WEIGHT} $weight $freq_col
      sed s/{${study_id}_WEIGHT}/${weight}/g metal_templates/f3_gwas_females_chrX.mi \
        | sed s/{${study_id}_FREQ}/${freq_col}/g \
        > metal_templates/f3_gwas_females_chrX_tmp.mi
      mv metal_templates/f3_gwas_females_chrX_tmp.mi metal_templates/f3_gwas_females_chrX.mi
    fi
  done < config/study_weights_chrX.tsv
fi
...
# Don't need to do for X chr


#Copy metal file and sumstats into bychr folder
cp metal_templates/f3_gwas_${RUNTYPE}_chrX.mi metal_scripts
ls metal_scripts/f3_gwas_${RUNTYPE}_chrX.mi > metafilelist.txt
#ls  metal_scripts/f3_gwas_nov1_2021_ehr_only.mi_* > metafilelist.txt #EHR studies only
#ls  metal_scripts/f3_gwas_nov1_2021_pgc_only.mi_* > metafilelist.txt #PGC studies only

#User: Give a name for the error log files
dataset=PTSD_F3_v4_${RUNTYPE}_chrX_nov1_2021
...
mv metal_results/eur_ptsd_pcs_v4_chrX_nov1_2021_1.tbl metal_results/eur_ptsd_pcs_v4_chrX_nov1_2021.tbl
mv metal_results/eur_ptsd_pcs_v4_chrX_nov1_2021_1.tbl.info metal_results/eur_ptsd_pcs_v4_chrX_nov1_2021.tbl.info
...
cat metal_results/eur_ptsd_pcs_v4_chrX_nov1_2021.tbl | awk -v totalN=$totalN -v percentN=$percentN '{ if (NR ==1 || ($6 >= 0.01 && $6 <= 0.99 && $3 != "MarkerName" && $10 >= percentN*totalN)) print $1,$2,$3,$4,$5,$6,$10,$11,$12}'   | grep -v : | sort -g -k 9 | gzip > results_filtered/eur_ptsd_pcs_v4_chrX_nov1_2021.fuma.gz

###LDSC analysis
conda activate ldsc

#Filter data down to just LDSC SNPs then munge - need ld data for X
# LC_ALL=C join -1 1 -2 3  <(awk '{print $1}' /home/maihofer/trauma_gwas/ldsc-master/ldsc_data/eur_w_ld_chr/w_hm3.snplist.sorted | LC_ALL=C sort -k1b,1 ) <(cat metal_results/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl | awk '{if (NR == 1) $3="SNP"; print}'   | LC_ALL=C sort -k3b,3 ) | gzip > results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl.premunge.gz
# python2 /home/maihofer/trauma_gwas/ldsc-master/munge_sumstats.py --sumstats results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl.premunge.gz  --N-col Weight --out results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl.munge.gz #add --N-col OBS_CT for the sample size
#
# #Get format for LDhub
# zcat results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.fuma.gz | awk '{if (NR==1) print "CHR","BP","SNP","A1","A2","FRQ","N","Z","P"; else print $0}' > results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl.munge.txt
# # zip results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl.munge.txt.zip results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl.munge.txt
# gzip -f results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl.munge.txt
#
# #Run LDSC
# python2 /home/maihofer/trauma_gwas/ldsc-master/ldsc.py \
# --h2 results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl.munge.gz.sumstats.gz \
...
# --out results_filtered/eur_ptsd_pcs_v4_${RUNTYPE}_nov1_2021.tbl.munge.gz.tbl.ldsc
